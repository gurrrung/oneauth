...
router.get(
  ...
  async (req, res) => {
    ...
    const trustedClient = req.client && req.client.trusted
    try {
  ...
  async (req, res) => {
    if (req.user && req.user.id) {
      ...
      if (req.query.include) {
        const includedAccounts = req.query.include.split(',')
        for (const ia of includedAccounts) {
          ...
          switch (ia) {
            ...
            case 'twitter':
              includes.push({
                ...
                attributes: { exclude: ['token', 'tokenSecret'] },
              })
              break
            case 'github':
              includes.push({
...
router.get(
  ...
  async (req, res) => {
    const includes = [
      {
        model: models.Demographic,
        include: [
          { model: models.Address, include: [models.State, models.Country] },
        ],
      },
    ]
    ...
    try {
      ...
      req.flash(
        'error',
        'Something went wrong trying to query address database'
      )
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!createdUser) {
        return res
          .status(400)
          .json({ error: 'Error creating account! Please try in some time' })
      }
      ...
      const setNewPassword = await models.Resetpassword.create({
        key: uid(15),
      ...
      return res
        .status(400)
        .json({ error: 'Unsuccessful registration. Please try again.' })
    }
  }
)
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: 'Please provide a Valid Contact Number.' })
      }
      ...
      if (user) {
        return res
          .status(400)
          .json({ error: 'Email already exists. Please try again.' })
      }
...
router.post(
  ...
  async (req, res, next) => {
    ...
    const user = await findUserById(req.body.oneauthId, [
      { model: models.Demographic, include: [models.Address] },
    ])

    if (!user) {
      return res
        .status(404)
        .json({ error: 'Resource to be updated not found.' })
    }

    if (
      !req.body.gradYear ||
      req.body.gradYear < 2000 ||
      req.body.gradYear > 2026
    ) {
    ...
    const userWithVerifiedNumber = await findUserByParams({
      verifiedmobile: `${req.body.dial_code}-${req.body.mobile_number}`,
    })

    // Check if mobile number to be updated, is verified with any other account
    if (
      userWithVerifiedNumber &&
      user.get().id !== userWithVerifiedNumber.get().id
    ) {
      return res.status(400).json({
        error: `${
          req.body.mobile_number
        } is already associated with coding blocks account ${
          userWithVerifiedNumber.get().id
        }`,
      })
    }
    ...
    try {
      ...
      const updatedUserDemographics = await findUserById(req.body.oneauthId, [
        models.Demographic,
      ])
      ...
      const addressOptions = {
        ...
        email: req.body.addressEmail
          ? req.body.addressEmail.toLowerCase()
          : updatedUserDemographics.get().email,
        pincode: req.body.pincode,
        street_address: req.body.street_address,
        landmark: req.body.landmark,
        city: req.body.city,
    ...
    try {
      const result = await sequelize.transaction(async (t) => {
        ...
        await upsertDemographic(
          ...
          req.body.collegeId ? +req.body.collegeId : demographic.collegeId,
          req.body.branchId ? +req.body.branchId : demographic.branchId,
          req.body.otherCollege
            ? req.body.otherCollege
            : demographic.otherCollege,
          { transaction: t }
        )

        // TODO: Dukaan credit hook
        if (
          user.demographic.collegeId === 1 &&
          !user.demographic.otherCollege && // OLD State
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await findUserByParams({
      ...
      return res.status(400).json({
        error: `Email already verified with codingblocks account ID:${user.get(
          'id'
        )}`,
      })
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (!user) {
        ...
        return res.status(400).json({
          error: 'This email is not registered with this codingblocks account.',
        })
      }
      await createVerifyEmailEntry(
        user,
        true,
