...
router.post('/', cel.ensureLoggedIn('/login'), async (req, res) => {
  if (
    ...
    try {
      ...
      const number = options.dial_code + options.mobile_number
      if (!validateNumber(parseNumberByCountry(number, req.body.countryId))) {
        ...
        return res.redirect(
          `/address/add${returnTo}` ? `?returnTo=${returnTo}` : ''
        )
      }
  ...
  try {
    await db.transaction(async (t) => {
      if (req.body.primary === 'on') {
        const demo = await findDemographic(req.user.id)
        const demoId = demo.id
        await updateAddressbyDemoId(demoId, { primary: false })
      }

      const number = req.body.dial_code + req.body.mobile_number
      if (!validateNumber(parseNumberByCountry(number, req.body.countryId))) {
        req.flash('error', 'This number does not exist in your country.')
        return res.redirect('/address/add')
      }
