  ...
  async (req, res, next) => {
    try {
      ...
      if (user) {
        res
          .status(422)
          .json({ error: `Username ${req.query.username} is not available.` })
      } else if (
        ...
        res
          .status(422)
          .json({ error: `Username ${req.query.username} is not valid.` })
      } else {
        res
          .status(200)
          .json({ success: `Username ${req.query.username} is available.` })
      }
    } catch (error) {
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (user) {
        res.status(422).json({
          error: `Email ${req.query.email} is already registered on oneauth.`,
        })
      } else if (!emailRegex.test(req.query.email)) {
        res
          .status(422)
          .json({ error: `${req.query.email} is not a valid email.` })
      } else {
        res
          .status(200)
          .json({ success: `Email ${req.query.email} is available.` })
      }
    } catch (error) {
      Raven.captureException(error)
