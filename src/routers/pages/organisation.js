...
router.get('/', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
    ...
    return res.render('/organisation/all', { organisations })
  } catch (error) {
...
router.get('/add', cel.ensureLoggedIn('/login'), (req, res, next) =>
  res.render('organisation/add')
)

router.get('/:id', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
    const organisation = await findOrganisationById(req.params.id)
    if (!organisation) {
      return res.send('Invalid Organisation Id')
    }
    return res.render('organisation/id', { organisation })
  } catch (error) {
    Raven.captureException(error)
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      return res.render('organisation/edit', { organisation })
    } catch (error) {
      Raven.captureException(error)
      req.flash('error', 'Error Getting Organisation')
      res.redirect('/users/me/organisations')
