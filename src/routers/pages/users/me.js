...
const { findAllOrganisations } = require('../../../controllers/organisation')
const {
  findAllBranches,
  findAllColleges,
  findAllCountries,
...
router.get('/', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
...
router.get('/edit', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
...
router.post('/edit', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  ...
  try {
    ...
    if (
      user.verifiedmobile &&
      user.verifiedmobile !== `${req.body.dial_code}-${req.body.mobile_number}`
    ) {
...
router.get('/clients', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
