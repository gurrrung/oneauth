...
const { findUserByParams, updateUserByParams } = require('../controllers/user')
const {
...
router.post(
  ...
  async (req, res) => {
    try {
      if (!req.user.email) {
        await updateUserByParams({ id: req.user.id }, { email: req.body.email })
      }
      const user = await findUserByParams({
      ...
      if (!user) {
        ...
        req.flash(
          'error',
          'The email id entered is not registered with this codingblocks account. Please enter your registered email.'
        )
        return res.redirect('/users/me')
      }
...
router.get('/key/:key', (req, res) => {
  ...
  findVerifyEmailEntryByKey(req.params.key)
    ...
    .then((verifiedUser) => {
      if (verifiedUser) {
        try {
          eventUserUpdated(verifiedUser.id).catch(
    ...
    .catch((err) => {
      ...
      req.flash(
        'error',
        'There was some problem verifying your email. Please try again.'
      )
      return res.redirect('/')
    })
})
