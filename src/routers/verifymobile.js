...
route.get('/', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
    ...
    const lastOTP = await models.VerifyMobile.findOne({
      // Find the last OTP sent to this number.
      where: {
    ...
    if (
      !lastOTP ||
      new Date(lastOTP.dataValues.createdAt).getTime() <
      ...
      createAndSendOTP(user.get('mobile_number'), key, 'Mobile Verification')
        .then((body) => {
          debug(body)
        })
        .catch((error) => {
...
route.post('/verify', cel.ensureLoggedIn('/login'), async (req, res) => {
  try {
    ...
    if (user) {
      ...
      req.flash(
        'error',
        `Mobile number already verified with codingblocks account ID:${user.get(
          'id'
        )}`
      )
    ...
    if (
      key.get('key') === req.body.otp &&
      !new Date(key.dataValues.createdAt).getTime() <
        new Date().getTime() - 10 * 60 * 1000
    ) {
...
route.post('/resend', cel.ensureLoggedIn('/login'), async (req, res, next) => {
  try {
    ...
    if (!user) {
      req.flash('Invalid user, Please login.')
      res.redirect('/login')
    } else if (user.get('verifiedmobile')) {
      // Mobile Number already verified, take person to profile page
      req.flash(
        'error',
        `Mobile number already verified with codingblocks account ID:${user.get(
    ...
    createAndSendOTP(user.mobile_number, key, 'Mobile Verification')
      .then((body) => {
        debug(body)
      })
      .catch((error) => {
