...
router.post(
  '/',
  makeGaEvent('attempt', 'signup', 'local'),
  async (req, res) => {
    ...
    try {
      ...
      if (req.body.refcode) {
        const userReferredBy = await findUserByParams({
      ...
      const includes = [{ model: models.User, include: [models.Demographic] }]

      try {
        const userLocal = await createUserLocal(query, passhash, includes)
        if (!userLocal) {
        ...
        req.flash(
          'error',
          `Error creating account! ${userCreationError.message}`
        )
        req.visitor
          .event({
            ea: 'unsuccessful',
