...
const {
  isValidPasswordForUser,
  isValidOtpForUser,
  makeTempOTPUserPermanent,
 ...
 */
server.grant(
  oauth.grant.code(async (client, redirectURL, user, ares, done) => {
 ...
 */
server.grant(
  oauth.grant.token(async (client, user, ares, done) => {
    try {
 ...
 */
server.exchange(
  oauth.exchange.code(async (client, code, redirectURI, done) => {
    try {
      ...
      for (const url of client.callbackURL) {
...
server.exchange(
  oauth.exchange.password(async (client, username, password, done) => {
    try {
...
const authorizationMiddleware = [
  ...
  server.authorization(
    async (clientId, callbackURL, done) => {
...
server.exchange(
  oauth.exchange.clientCredentials(async (client, scope, done) => {
    // Validate the client
    try {
...
const decisionMiddleware = [cel.ensureLoggedIn('/login'), server.decision()]
