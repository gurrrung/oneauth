...
async function createUserLocal(userParams, pass, includes) {
  ...
  let userLocal
  try {
    userLocal = await UserLocal.create(
      { user: userParams, password: pass },
      { include: includes }
    )
  } catch (err) {
...
function findUserForTrustedClient(trustedClient, userId) {
  return User.findOne({
    attributes: trustedClient
      ? undefined
      : ['id', 'username', 'photo', 'graduationYear'],
    where: { id: userId },
    include: {
      model: Demographic,
...
function generateFilter(filterArgs) {
  ...
  if (filterArgs.verified) {
    const verify = filterArgs.verified === 'true'
    if (verify) {
...
function findAllUsersWithFilter(trustedClient, filterArgs) {
  return User.findAll({
    attributes: trustedClient
      ? undefined
      : ['id', 'username', 'email', 'firstname', 'lastname', 'mobile_number'],
