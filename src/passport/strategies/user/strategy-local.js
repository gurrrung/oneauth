  ...
  async (req, username, password, cb) => {
    ...
    try {
      ...
      const userLocal =
        ...
        userLocals.find((userLocal) => userLocal.user.verifiedemail) ||
        userLocals[0]

      if (
        !userLocal.user.verifiedemail &&
        userLocal.user.username !== username
      ) {
        await createVerifyEmailEntry(userLocal.user, true, '/users/me')
        return cb(null, false, {
          message:
            'Unverified Email. Click on the link sent to your email address',
        })
      }
