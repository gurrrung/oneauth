    ...
    try {
      if (oldUser) {
        ...
        /*
                ...
                */
        const glaccount = await models.UserGoogle.findOne({
          where: { id: profileJson.sub },
        })
        if (glaccount) {
          throw new Error(
            `Your Google account is already linked with codingblocks account Id: ${glaccount.dataValues.userId}`
          )
        } else {
          ...
          if (user) {
            ...
            return cb(null, false, {
              message: 'Could not retrieve existing Google linked account',
            })
          }
        }
      } else {
        ...
        if (!userGoogle) {
          ...
          const existCount = await models.User.count({
            where: { username: profileJson.username },
          })
